...
{%  set getLatestCRMTable_query %}
    ...
    Select table_id, creation_time  FROM dataraw.brightdoorSpringIsland.__TABLES__
    WHERE creation_time = 
        (
        SELECT MAX(creation_time) AS maxCRMCreate
        FROM dataraw.brightdoorSpringIsland.__TABLES__
...
FROM `dataraw.brightdoorSpringIsland.{{CRMtable}}` WHERE emailaddress != 'nan'