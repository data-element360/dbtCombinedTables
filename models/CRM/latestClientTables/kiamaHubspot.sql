...
{%  set getLatestContactTable_query %}
    ...
    Select table_id, creation_time  FROM dataraw.{{dataSpace}}.__TABLES__
    WHERE creation_time = 
        (
        SELECT MAX(creation_time) AS maxContactCreate
        FROM dataraw.{{dataSpace}}.__TABLES__
        WHERE CONTAINS_SUBSTR(table_id ,"{{client}}_contacts")
        )

    AND 
        CONTAINS_SUBSTR(table_id ,"{{client}}_contacts")
...
{%  set getLatestDealTable_query %}
    ...
    WHERE creation_time = 
        ...
        FROM dataraw.{{dataSpace}}.__TABLES__
        WHERE CONTAINS_SUBSTR(table_id ,"{{client}}_deals")
        )

    AND 
        CONTAINS_SUBSTR(table_id ,"{{client}}_deals")
...
SELECT 
    ...
    FROM `dataraw.{{dataSpace}}.{{contact_table}}`
),
hubspotDeals AS (
...
SELECT 
    ...
    hs_object_id,
FROM (
    (SELECT CONCAT(pipeline,dealstage) AS stageConcat, * FROM `dataraw.{{dataSpace}}.{{deal_table}}`) Deals
    LEFT JOIN 
    (SELECT CONCAT(pipeline,stageId) AS stageConcat, * FROM `dataraw.{{dataSpace}}.{{client}}_pipeline`) Pipeline
...
associations AS (
SELECT * FROM `dataraw.{{dataSpace}}.{{client}}_dealToContactAssociation`
),
...
SELECT "{{client}}" AS client, * EXCEPT(hs_object_id) FROM allUnion